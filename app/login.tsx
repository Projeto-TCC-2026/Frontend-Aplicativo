  ...
  return (
    <Screen className="items-center justify-center bg-neutral-100 p-5 dark:bg-theme-dark-background">
      <View className="w-full max-w-[480px] gap-4 rounded-[14px] bg-white p-6 dark:bg-theme-dark-surface">
        <Text className="font-display text-[28px] font-extrabold text-neutral-900 dark:text-theme-dark-text-primary">Recupera Saúde</Text>
        <Text className="font-body text-[15px] leading-[22px] text-neutral-700 dark:text-theme-dark-text-secondary">Entre para acessar seu check-in diário.</Text>
        ...
        <TextField disabled={loading} label="Senha" onChangeText={setPassword} placeholder="Sua senha" type="password" value={password} />
        {errorMessage ? <Text accessibilityRole="alert" className="font-body text-[13px] text-semantic-critical">{errorMessage}</Text> : null}
        <Button fullWidth loading={loading} loadingText="Entrando..." onPress={() => void submit()}>Entrar</Button>
        <Text className="my-3 font-body text-[13px] text-right text-neutral-700 dark:text-theme-dark-text-secondary underline" onPress={() => router.push('/esqueci-a-senha')}>Esqueci a senha</Text>
      </View>
    </Screen>
  );
}
