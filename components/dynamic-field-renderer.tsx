...
export function DynamicFieldRenderer({ field, value, onChange, error, disabled = false, onPickPhoto, renderPhotoPreview }: DynamicFieldRendererProps) {
  ...
  return <View className="mb-[18px] gap-1.5"><Text className="font-body text-[15px] font-semibold text-neutral-900 dark:text-theme-dark-text-primary">{label}</Text>{field.description ? <Text className="font-body text-[13px] text-neutral-500 dark:text-theme-dark-text-tertiary">{field.description}</Text> : null}{renderControl(field, value, onChange, error, disabled, onPickPhoto, renderPhotoPreview, colorScheme === 'dark')}{field.unit && field.dataType !== 'PHOTO' ? <Text className="font-body text-xs text-neutral-500 dark:text-theme-dark-text-tertiary">{field.unit}</Text> : null}{error ? <Text accessibilityRole="alert" className="font-body text-xs text-semantic-critical">{error}</Text> : null}</View>;
}

function renderControl(field: DynamicField, value: DynamicFieldValue, onChange: (value: DynamicFieldValue) => void, error: string | undefined, disabled: boolean, onPickPhoto?: () => void, renderPhotoPreview?: (photoUrl: string) => ReactNode, isDark = false) {
  if (field.dataType === 'BOOLEAN') return <View className="min-h-12 flex-row items-center justify-between"><Text className="font-body text-[15px] text-neutral-700 dark:text-theme-dark-text-secondary">{value === 'true' ? 'Sim' : value === 'false' ? 'Não' : 'Não respondido'}</Text><Switch disabled={disabled} value={value === 'true'} onValueChange={(nextValue) => onChange(String(nextValue))} /></View>;
  if (field.dataType === 'SCALE') { const { min, max } = getScaleRange(field); const current = value != null ? Number(value) : min; return <View className="gap-2"><Text accessibilityLiveRegion="polite" className="self-center font-data text-2xl font-semibold text-brand-dark dark:text-theme-dark-action">{current}</Text><Slider disabled={disabled} maximumValue={max} minimumValue={min} onValueChange={(nextValue) => onChange(String(Math.round(nextValue)))} step={1} style={{ width: '100%', height: 40 }} maximumTrackTintColor={isDark ? colors.darkSliderTrack : colors.neutral300} minimumTrackTintColor={colors.primary} thumbTintColor={colors.primary} value={current} /><View className="flex-row justify-between"><Text className="font-body text-xs text-neutral-500 dark:text-theme-dark-text-tertiary">{min}</Text><Text className="font-body text-xs text-neutral-500 dark:text-theme-dark-text-tertiary">{max}</Text></View></View>; }
