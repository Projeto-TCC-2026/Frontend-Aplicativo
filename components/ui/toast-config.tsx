...
function renderToast({ type, text1, text2 }: ToastConfigParams<unknown>) {
  ...
  return (
    <View className={`mx-4 gap-1 rounded-[12px] border-l-4 bg-white p-4 shadow-lg dark:bg-theme-dark-surface ${borderClass}`}>
      {text1 ? <Text className="font-display text-[15px] font-bold text-neutral-900 dark:text-theme-dark-text-primary">{text1}</Text> : null}
      {text2 ? <Text className="font-body text-[13px] text-neutral-700 dark:text-theme-dark-text-secondary">{text2}</Text> : null}
    </View>
  );
}
